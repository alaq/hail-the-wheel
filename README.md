...
* [`src/clj/hail_the_wheel/`](src/clj/hail_the_wheel/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/hail_the_wheel/`](src/cljs/hail_the_wheel/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/hail_the_wheel/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `hail-the-wheel` project root directory
...
in the [`src/clj/hail_the_wheel/`](src/clj/hail_the_wheel/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/hail_the_wheel/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns hail-the-wheel.example
  (:require [hail-the-wheel.config :as config])
